
(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(column-number-mode t)
 '(custom-safe-themes
   '("acfe7ff6aacb9432f124cde4e35d6d2b4bc52916411de73a6ccded9750c9fa97"
	 "b8bd60a23b9e2f08b0c437231ee84f2dacc70fdc4d5a0fb87229bb9926273fdd"
	 "dc15dbd4b0a00c64610fd4379a89424e0be1b418f09457e0f062cac931e8ca82"
	 "3d39093437469a0ae165c1813d454351b16e4534473f62bc6e3df41bb00ae558"
	 "cbd85ab34afb47003fa7f814a462c24affb1de81ebf172b78cb4e65186ba59d2"
	 "01a9797244146bbae39b18ef37e6f2ca5bebded90d9fe3a2f342a9e863aaa4fd"
	 "9e296dbc86374778cca0f22cfd7cd44d35e7c2e678085417be97251ce7c75dcc"
	 "8f5b54bf6a36fe1c138219960dd324aad8ab1f62f543bed73ef5ad60956e36ae"
	 "ffba0482d3548c9494e84c1324d527f73ea4e43fff8dfd0e48faa8fc6d5c2bc7"
	 "d0fd069415ef23ccc21ccb0e54d93bdbb996a6cce48ffce7f810826bb243502c"
	 "571661a9d205cb32dfed5566019ad54f5bb3415d2d88f7ea1d00c7c794e70a36"
	 "b54376ec363568656d54578d28b95382854f62b74c32077821fdfd604268616a"
	 "f5f80dd6588e59cfc3ce2f11568ff8296717a938edd448a947f9823a4e282b66"
	 "8d8207a39e18e2cc95ebddf62f841442d36fcba01a2a9451773d4ed30b632443"
	 "37b6695bae243145fa2dfb41440c204cd22833c25cd1993b0f258905b9e65577"
	 "e14884c30d875c64f6a9cdd68fe87ef94385550cab4890182197b95d53a7cf40"
	 "00cec71d41047ebabeb310a325c365d5bc4b7fab0a681a2a108d32fb161b4006"
	 "e8ceeba381ba723b59a9abc4961f41583112fc7dc0e886d9fc36fa1dc37b4079"
	 "b9761a2e568bee658e0ff723dd620d844172943eb5ec4053e2b199c59e0bcc22"
	 "6a5584ee8de384f2d8b1a1c30ed5b8af1d00adcbdcd70ba1967898c265878acf"
	 "e4a702e262c3e3501dfe25091621fe12cd63c7845221687e36a79e17cf3a67e0"
	 "2b501400e19b1dd09d8b3708cefcb5227fda580754051a24e8abf3aff0601f87"
	 "dd4582661a1c6b865a33b89312c97a13a3885dc95992e2e5fc57456b4c545176"
	 "56044c5a9cc45b6ec45c0eb28df100d3f0a576f18eef33ff8ff5d32bac2d9700"
	 "014cb63097fc7dbda3edf53eb09802237961cbb4c9e9abd705f23b86511b0a69"
	 "88f7ee5594021c60a4a6a1c275614103de8c1435d6d08cc58882f920e0cec65e"
	 "1f292969fc19ba45fbc6542ed54e58ab5ad3dbe41b70d8cb2d1f85c22d07e518"
	 "b754d3a03c34cfba9ad7991380d26984ebd0761925773530e24d8dd8b6894738"
	 "2078837f21ac3b0cc84167306fa1058e3199bbd12b6d5b56e3777a4125ff6851"
	 "0c83e0b50946e39e237769ad368a08f2cd1c854ccbcd1a01d39fdce4d6f86478"
	 "38c0c668d8ac3841cb9608522ca116067177c92feeabc6f002a27249976d7434"
	 "f4d1b183465f2d29b7a2e9dbe87ccc20598e79738e5d29fc52ec8fb8c576fcfd"
	 "4b6cc3b60871e2f4f9a026a5c86df27905fb1b0e96277ff18a76a39ca53b82e1"
	 "b5fd9c7429d52190235f2383e47d340d7ff769f141cd8f9e7a4629a81abc6b19"
	 "4780d7ce6e5491e2c1190082f7fe0f812707fc77455616ab6f8b38e796cbffa9"
	 "cc0dbb53a10215b696d391a90de635ba1699072745bf653b53774706999208e3"
	 "3e335d794ed3030fefd0dbd7ff2d3555e29481fe4bbb0106ea11c660d6001767"
	 "aec7b55f2a13307a55517fdf08438863d694550565dee23181d2ebd973ebd6b8"
	 "34cf3305b35e3a8132a0b1bdf2c67623bc2cb05b125f8d7d26bd51fd16d547ec"
	 "4594d6b9753691142f02e67b8eb0fda7d12f6cc9f1299a49b819312d6addad1d"
	 "9f297216c88ca3f47e5f10f8bd884ab24ac5bc9d884f0f23589b0a46a608fe14"
	 "d6b934330450d9de1112cbb7617eaf929244d192c4ffb1b9e6b63ad574784aad"
	 "4b026ac68a1aa4d1a91879b64f54c2490b4ecad8b64de5b1865bca0addd053d9"
	 "88cb0f9c0c11dbb4c26a628d35eb9239d1cf580cfd28e332e654e7f58b4e721b"
	 "21e3d55141186651571241c2ba3c665979d1e886f53b2e52411e9e96659132d4"
	 "69f7e8101867cfac410e88140f8c51b4433b93680901bb0b52014144366a08c8"
	 "00a9bb90413c4e218f02287db09e5dae9f6080890dd641ec0d6ff83d28d1335f"
	 "f60404efc40b646a6a742d833c7097f9225550288565f945ec990d343c1a22ff"
	 "36ca8f60565af20ef4f30783aa16a26d96c02df7b4e54e9900a5138fb33808da"
	 "c9ddf33b383e74dac7690255dd2c3dfa1961a8e8a1d20e401c6572febef61045"
	 "bf798e9e8ff00d4bf2512597f36e5a135ce48e477ce88a0764cfb5d8104e8163"
	 "e27c9668d7eddf75373fa6b07475ae2d6892185f07ebed037eedf783318761d7"
	 "631c52620e2953e744f2b56d102eae503017047fb43d65ce028e88ef5846ea3b"
	 default))
 '(display-line-numbers-type 'relative)
 '(helm-command-prefix-key "C-;")
 '(lisp-body-indent 4)
 '(org-format-latex-options
   '(:foreground default :background default :scale 2 :html-foreground
				 "Black" :html-background "Transparent" :html-scale
				 1.0 :matchers ("begin" "$1" "$" "$$" "\\(" "\\[")))
 '(package-selected-packages
   '(ace-pinyin ada-mode almost-mono-themes ample-theme cape
				chatgpt-shell command-log-mode company corfu
				corfu-prescient dakrone-light-theme dired-open
				dired-single doom-themes dot-mode dracula-theme eglot
				emacsql-sqlite eterm-256color evil evil-collection
				evil-surround expand-region flx-ido flycheck
				flycheck-posframe fzf general gnuplot gnuplot-mode
				gruber-darker-theme haskell-mode icomplete-vertical
				ivy-prescient kanagawa-theme lispy lua-mode magit
				mindre-theme minimal-theme mips-mode modus-themes
				move-text mu4e multiple-cursors nim-mode no-literring
				orderless org-bullets org-modern org-ref org-roam
				org-roam-ui ox-rfc pomodoro prescient
				rainbow-identifiers rainbow-mode rfc-mode shift-number
				smex stumpwm-mode tao-theme treesit-auto tuareg
				undo-tree visible-mark visual-fill-column vterm
				which-key writeroom-mode yaml-mode yasnippet
				yasnippet-capf yasnippet-snippets zig-mode zotxt))
 '(window-divider-mode t))

(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(fixed-pitch ((t (:family "Iosevka"))))
 '(line-number ((t (:inherit default :foreground "dim gray" :slant italic :weight normal))))
 '(variable-pitch ((t (:family "ETBembo")))))
